// app/(public)/page.js (หรือ app/(public)/products/page.js)
'use client';
...
// Utility function (debounce) - Define it here or import
...
export default function HomePage() {
  ...
  // --- Buy Product Logic (เหมือนเดิมจากตัวอย่างก่อน) ---
  const handleBuyClick = async (product) => {
     if (!product || parseInt(product.Stocks) <= 0) return;
     // ... (โค้ด Swal form และ API calls สำหรับการสั่งซื้อ) ...
     // ดูตัวอย่างโค้ดในส่วน `app/(public)/products/page.js` ก่อนหน้านี้
     // ตรวจสอบให้แน่ใจว่าเรียก API endpoints ถูกต้อง:
     // POST /api/customers
     // POST /api/orders
     // POST /api/orders/details
     // PUT /api/products/[id] (สำหรับลด stock)

    console.log("Buy button clicked for:", product.Fur_ID); // Placeholder
    // --- START: ใส่โค้ด Swal Form และ API Calls ที่นี่ ---
        const { value: formData, isConfirmed } = await Swal.fire({
            ...
            preConfirm: () => {
                ...
                const delivery = Swal.getPopup().querySelector('input[name="delivery"]:checked')?.value;
                const address = Swal.getPopup().querySelector('#swal-address').value;

                if (!name) return Swal.showValidationMessage('กรุณากรอกชื่อ');
                if (!phone || !/^[0-9]{10}$/.test(phone)) return Swal.showValidationMessage('เบอร์โทรไม่ถูกต้อง');
                if (!delivery) return Swal.showValidationMessage('กรุณาเลือกวิธีรับสินค้า');
                if (delivery === 'delivery' && !address) return Swal.showValidationMessage('กรุณากรอกที่อยู่');

                return { name, phone, delivery, address: delivery === 'delivery' ? address : null };
            },
         ...
         if (isConfirmed && formData) {
            ...
            try {
                ...
                await axios.post('/api/customers', { CustID: custId, CustName: formData.name, Phone: formData.phone, Address: formData.address });
                await axios.post('/api/orders', { InvoiceID: invoiceId, CustID: custId, TotalPrice: totalPrice, deliveryMethod: formData.delivery, orderDate: new Date().toISOString().split('T')[0] });
                await axios.post('/api/orders/details', { OrderDetailID: orderDetailId, InvoiceID: invoiceId, Fur_ID: product.Fur_ID, Qty: 1, Price: product.Prices });
                await axios.put(`/api/products/${product.Fur_ID}`, { stockChange: -1 }); // Assume API handles stock update

                Swal.fire('สำเร็จ!', `สั่งซื้อสินค้าเรียบร้อย เลขที่: ${invoiceId}`, 'success');
    ...
    // --- END: ใส่โค้ด Swal Form และ API Calls ที่นี่ ---
  };
      ...
      <div className="container my-4">
        ...
        <div className="card card-body shadow-sm mb-4 filters">
           <div className="row g-3 align-items-center">
                ...
                <div className="col-md-3">
                     <select className="form-select form-select-sm" value={materialFilter} onChange={(e) => setMaterialFilter(e.target.value)}>
                 ...
                 <div className="col-md-1 text-end">
                      <button
                          ...
                          onClick={() => { setSearchTerm(''); setTypeFilter(''); setMaterialFilter(''); }}
                          title="ล้างตัวกรอง"
          ...
          <div className="row row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-lg-4 g-4">
            {filteredProducts.length > 0 ? (
              ...
              <div className="col-12 text-center mt-5">
                <h4><i className="fas fa-search me-2"></i>ไม่พบสินค้า</h4>
        ...
        <div className="container">
            <div className="row">
                <div className="col-md-6 mb-4 mb-md-0">
                    <h4 className="mb-4"><i className="fas fa-phone-alt me-2"></i>ติดต่อเรา</h4>
                    <p><i className="fas fa-map-marker-alt me-2 text-secondary" style={{width: '20px'}}></i>Khlong Sam, Khlong Luang District, Pathum Thani 12120</p>
                    <p><i className="fas fa-phone me-2 text-secondary" style={{width: '20px'}}></i>02-123-4567</p>
                    <p><i className="fas fa-clock me-2 text-secondary" style={{width: '20px'}}></i>เปิดทุกวัน 9:00 - 20:00 น.</p>
                    {/* เพิ่ม Social Media Links ถ้ามี */}
                </div>
                <div className="col-md-6">
                    <h4 className="mb-4"><i className="fas fa-map-marked-alt me-2"></i>แผนที่ร้าน</h4>
                    <div className="ratio ratio-16x9 rounded overflow-hidden shadow-sm">
                         <iframe
                            // FIXME: Replace with your actual Google Maps embed URL
